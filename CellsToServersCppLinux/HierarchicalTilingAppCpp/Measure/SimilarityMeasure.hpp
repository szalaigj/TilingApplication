...
namespace Measure
{
	...
	inline double SimilarityMeasure<T>::computeMeasureForRegion(Coords * coords)
	{
		...
		int spaceDimension = this->getAuxData().getSpaceDimension();
		int histogramResolution = this->getAuxData().getHistogramResolution();
		int * indicesArrayOfRegion =
			this->transformator.determineIndicesArray(spaceDimension, coords->getExtendedIndicesArray());
		int binNOInRegionWithoutZeroHeft = 0;
		int * indicesArrayOfBin;
		for (indicesArrayOfBin =
			this->transformator.determineFirstContainedIndicesArray(spaceDimension, indicesArrayOfRegion);
			indicesArrayOfBin != nullptr;
			indicesArrayOfBin = this->transformator.determineNextContainedIndicesArray(spaceDimension,
				indicesArrayOfRegion, indicesArrayOfBin))
		{
			int currentCellIdx = this->transformator.calculateCellIdx(spaceDimension,
						histogramResolution, indicesArrayOfBin);
			int binValue = (int)(this->getAuxData().getHistogram())[currentCellIdx];
			if (binValue > 0)
			{
				binNOInRegionWithoutZeroHeft++;
				measureForRegion += this->computeMeasureForBin(indicesArrayOfBin, indicesArrayOfRegion);
			}
		}
